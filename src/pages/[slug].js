...
const Blog = ({ blog, author, comments, categories, recentPostsData, relatedPostsData }) => {
    ...
    return (
        <>
            <SeoHead yoastMeta={blog.yoastMeta[0]} blog={blog} author={author} />
...
const BlogDetail = ({ blog, author, comments, relatedPostsData }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/posts/comments`, {
                ...formData,
                postId: blog.ID
            });
            ...
            <div className="col-lg-9 col-md-12 col-sm-12 col-xs-12">
                <div className="page-wrapper">
                    <div className="blog-title-area">
                        <span className="bg-aqua" style={{ display: 'inline', padding: '0 0.2rem', background: '#74a044 !important', borderRadius: '5px' }}>
                        ...
                        <div className="blog-meta big-meta">
                            <small>{format(new Date(blog.post_date), 'MMMM d, yyyy')}</small>
                            <small>  <Link href={`/author/${blog.post_author}`} title>
                                {blog.author_name}
                            </Link></small>
                        </div>{/* end meta */}
                        ...
                        <div className="tag-cloud-single">
                            <span>Tags</span>
                            {
                                blog && blog.tags.map((tag) => (
                                    <small key={tag.slug}><a href="#" title={tag.name}>{tag.name}</a></small>
                                ))
                            }
                    ...
                    <div className="custombox authorbox clearfix">
                        ...
                        <div className="row">
                            ...
                            <div className="col-lg-10 col-md-10 col-sm-10 col-xs-12">
                                <h4><a href={`/author/${author.id}`}>{author.displayName}</a></h4>
                                <div dangerouslySetInnerHTML={{ __html: (author.description) }} />

                                <div className="topsocial">
                                    {author.socialHandles.facebook && (
                                        <a href={author.socialHandles.facebook} data-toggle="tooltip" data-animation="false"data-placement="bottom" title="Facebook">
                                            <i className="fa fa-facebook" />
                                        </a>
                                    )}
                                    {author.socialHandles.youtube && (
                                        <a href={author.socialHandles.youtube} data-toggle="tooltip" data-animation="false"data-placement="bottom" title="Youtube">
                                            <i className="fa fa-youtube" />
                                        </a>
                                    )}
                                    {author.socialHandles.pinterest && (
                                        <a href={author.socialHandles.pinterest} data-toggle="tooltip" data-animation="false"data-placement="bottom" title="Pinterest">
                                            <i className="fa fa-pinterest" />
                                        </a>
                                    )}
                                    {author.socialHandles.twitter && (
                                        <a href={author.socialHandles.twitter} data-toggle="tooltip" data-animation="false"data-placement="bottom" title="Twitter">
                                            <i className="fa fa-twitter" />
                                        </a>
                                    )}
                                    {author.socialHandles.instagram && (
                                        <a href={author.socialHandles.instagram} data-toggle="tooltip" data-animation="false"data-placement="bottom" title="Instagram">
                                            <i className="fa fa-instagram" />
                                        </a>
                                    )}
                                    {author.website && (
                                        <a href={author.website} data-toggle="tooltip" data-animation="false"data-placement="bottom" title="Website">
                    ...
                    <hr className="invis1" />
                    {filteredRelatedPosts.length > 0 && (<div className="custombox clearfix">
                                        ...
                                        <div className="blog-box">
                                            <div className="post-media">
                                                <a href={`/${post.post_name}`} title>
                                                    <Image src={post.thumbnail_url} alt={post.post_title} width={600} height={400} className="img-fluid" />
                                            ...
                                            <div className="blog-meta">
                                                <h4><a href={`/${post.post_name}`} title>{post.post_title}</a></h4>
                                                <small><a href={`/${post.post_name}`} title>{format(new Date(post.post_date), 'MMMM d, yyyy')}</a></small>
                                            </div>
                                        </div>
                                    </div>
                                ))
...
export async function getStaticProps(context) {
    ...
    try {
        const [postResponse, categoriesResponse, recentPostsResponse] = await Promise.all([
            axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/posts/${slug}`),
        ...
        let author = null;
        if (blog.post_author) {
            const authorResponse = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/authors/${blog.post_author}`);
            author = authorResponse.data.authors[0];
        }
        let relatedPostsData = null;

        if (blog) {
            const relatedPostsResponse = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/posts/related?categoryName=${postResponse.data.post.categories[0].name}`)
...
export async function getStaticPaths() {
    ...
    const slugs = response.data.data.posts.map(post => post.post_name);
    ...
    return {
        paths,
        fallback: 'blocking', // Use 'blocking' to serve a fallback version of the page while the new page is generated
    };
}
