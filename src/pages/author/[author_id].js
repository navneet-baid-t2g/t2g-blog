...
const AuthorPage = ({ initialPosts, initialPagination, categories, recentPostsData, authorId }) => {
    ...
    const fetchPosts = async (page, authorId) => {
        setLoading(true);
        try {
            const apiKey = 'test'; // Replace with your actual API key
            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/posts/author/${authorId}?page=${page}`, {
                headers: {
                    'x-api-key': apiKey,
                },
            });
...
// Function to fetch all author IDs
export async function getStaticPaths() {
    const apiKey = 'test'; // Replace with your actual API key

    try {
        const authorsResponse = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/authors`, {
            headers: { 'x-api-key': apiKey },
        });

        const authorsData = authorsResponse.data.authors;

        const paths = authorsData.map(author => ({
        ...
        console.error('Error fetching author IDs:', error);
        return {
            paths: [],
            fallback: false,
        };
...
export async function getStaticProps({ params }) {
    ...
    try {
        const [postsResponse, categoriesResponse, recentPostsResponse] = await Promise.all([
            axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/posts/author/${params.author_id}?page=${page}`, {
                headers: { 'x-api-key': apiKey },
            }),
        ...
        return {
            props: {
                ...
                recentPostsData,
                authorId: params.author_id, // Pass the author ID to the component
            },
